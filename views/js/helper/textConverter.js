...
define([
    ...
    /**
     * Gets the API that access to the labels
     * @returns {Promise}
     */
    function testcenterLabelsLoader() {
        if (!pendingPromise) {
            pendingPromise = new Promise(function (resolve, reject) {
                $.ajax({
                    url: url.route('getAll', 'TextConverter', 'taoTestCenterRostering'),
                    type: 'POST',
                    dataType: 'json'
                })
                    .done(function (data) {
                        labels = data.translations;
                        resolve(testcenterLabels);
                    })
                    .fail(reject);
            });
        }
